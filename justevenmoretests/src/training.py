...
def run_pytorch_hpo(model_type, data_splits, n_trials, hpo_seed):
    ...
    def objective(trial):
        ...
        for epoch in range(50):
            ...
            mdl.eval()
            with torch.no_grad():
                val_auc = roc_auc_score(y_v, mdl.predict_proba(X_v)[:, 1])
            mdl.train()
    ...
    if n_trials > 0:
        ...
        print(f"  Bester AUC: {study.best_value:.4f}")
    else:
...
def run_pytorch_final(model_type, data_splits, best_params, epochs, dataset_name, seed):
    ...
    for epoch in range(epochs):
        ...
        val_loss_w, val_loss_uw = _compute_val_loss(model, X_cal, y_cal, criterion)
        ...
        if val_loss_w < best_val_loss:
            ...
            ckpt.save(model, optimizer, epoch, -val_loss_w, is_best=True)
            patience_counter = 0
        else:
            patience_counter += 1

        print(f"  Ep {epoch + 1:03d} | Train {train_loss:.4f} | Val {val_loss_w:.4f} "
...
def run_tabnet_final(data_splits, best_params, epochs, dataset_name, seed):
    ...
    model.fit(
        X_train, y_train,
        eval_set=[(X_cal, y_cal)], eval_metric=['auc'],
...
def run_tree_hpo(model_type, data_splits, n_trials, hpo_seed):
    ...
    def objective(trial):
        ...
        if model_type == "xgboost":
            ...
            clf = XGBClassifier(**params, scale_pos_weight=scale_pos, eval_metric="auc",
            ...
            clf = LGBMClassifier(**params, is_unbalance=True, metric="auc",
            ...
            clf = CatBoostClassifier(**params, auto_class_weights="Balanced",
                                     eval_metric="AUC", random_seed=hpo_seed, verbose=0)
            clf.fit(X_hpo_tr, y_hpo_tr, eval_set=(X_hpo_val, y_hpo_val),
                    early_stopping_rounds=50, verbose=0)

        preds = clf.predict_proba(X_hpo_val)[:, 1]
        try:
            return roc_auc_score(y_hpo_val, preds)
        except ValueError:
            return 0.5
...
def run_tree_final(model_type, data_splits, best_params, dataset_name, seed):
    ...
    if model_type == "xgboost":
        ...
        model = LGBMClassifier(**best_params, is_unbalance=True, metric="auc",
    ...
    cal_auc = roc_auc_score(y_cal, model.predict_proba(X_cal)[:, 1])
